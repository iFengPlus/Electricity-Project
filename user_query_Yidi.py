...
# -*- coding: utf-8 -*-
"""
Created on Tue Feb 11 17:43:24 2025
...
def handle_callbacks(login_clicks, query_clicks, user_id, password, query_type):
    triggered_id = ctx.triggered_id  


    if triggered_id == "login-btn":
        user = next((u for u in user_data if u["userID"] == user_id and u["password"] == password), None)
        if user:
        ...
        created_at = datetime.datetime.strptime(user["timestamp"], "%Y-%m-%dT%H:%M:%S")
